/**
 * Beams — атмосферний WebGL-фон апки. Адаптовано з react-bits (Beams,
 * JS+CSS варіант, three/@react-three/fiber/drei) під TypeScript і нашу
 * палітру: фон сцени та базовий колір балок — `backgroundColor` (--ink),
 * підсвітка напрямленого світла — `lightColor` (--green за замовчуванням).
 * `dpr`/`active` — нові пропси для перфоманс-обмежень (керує AtmosphereBackground).
 */
...
function extendMaterial(
  ...
  if (defaults.roughness !== undefined) uniforms.roughness.value = defaults.roughness;
  if (defaults.metalness !== undefined) uniforms.metalness.value = defaults.metalness;
  if (defaults.envMap !== undefined) uniforms.envMap.value = defaults.envMap;
...
const MergedPlanes = forwardRef<THREE.Mesh, PlaneNoiseProps>(
  ({ material, width, count, height }, ref) => {
    ...
    useFrame((_, delta) => {
      ...
      if (mat instanceof THREE.ShaderMaterial) {
        // Імперативна анімація поза React-рендером (стандартний r3f-патерн
        // useFrame); не "мутація рендеру", тож безпечна.
...
export default function Beams({
  ...
  return (
    <Canvas dpr={dpr} frameloop={active ? "always" : "never"} className="beams-container">
