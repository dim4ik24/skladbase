...
def create_scheduler() -> AsyncIOScheduler:
    # next_run_time=зараз -> перший тік ОДРАЗУ при старті процесу, далі за
    # інтервалом як зазвичай. Без цього кожен деплой/рестарт (а вони бувають
    # частіше, ніж інтервал джоби) відкладає перший реальний прогін на весь
    # інтервал — для np_tracking це 10 хв мовчання після кожного рестарту.
    # Застосовано лише там, де негайний перший прогін безпечний: усі ці джоби
    # ідемпотентні за конструкцією (фільтр "ще не оброблено" + позначка після
    # обробки), тож зайвий тик на рестарті не дублює сповіщення/дії.
    now = datetime.now(UTC)

    scheduler = AsyncIOScheduler()
    scheduler.add_job(
        _run_np_tracking, "interval", minutes=10, id="np_tracking", next_run_time=now
    )
    scheduler.add_job(
    ...
    scheduler.add_job(
        _run_low_stock_scan, "interval", hours=1, id="low_stock_scan", next_run_time=now
    )
    # charge_due_card_subscriptions — БЕЗ негайного старту: реальне списання
    # грошей з картки. Навіть ідемпотентність за current_period_end не варта
    # ризику зайвого прогону фінансової джоби на кожному рестарті/деплої —
    # тут краще стабільний інтервал, ніж діагностична зручність.
    scheduler.add_job(
