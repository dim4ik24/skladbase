"""
SkladBase — вхідні вебхуки платіжних провайдерів (Стадії 5b/5c): WayForPay
...
Провайдери шлють вебхуки повторно (мережеві ретраї) — `record_payment` сам
ідемпотентний за `transaction_id` (Стадія 5c), тож повторна доставка тут не
обробляється окремо: просто викликаємо його щоразу як прийшов валідний
вебхук, дублікат не продовжить період вдруге.
"""
...
async def wayforpay_webhook(
    ...
    resolved = await _resolve_from_ref(session, order_reference)
    if resolved is None:
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={"ok": False})
    ...
    await SubscriptionService(session).record_payment(
        ...
        # orderReference — унікальний per-транзакція (ідемпотентність);
        # recToken — окремо, для крона charge_due_card_subscriptions.
        transaction_id=order_reference,
        recurring_token=data.get("recToken"),
    ...
    return JSONResponse(content=provider.build_accept_response(order_reference))
...
async def nowpayments_webhook(
    ...
    await SubscriptionService(session).record_payment(
        ...
        amount=result.amount,
        currency=result.currency,
        transaction_id=result.external_id,  # payment_id — унікальний per-транзакція
        recurring_token=None,  # крипта не авто-продовжується, токена немає
        is_recurring=result.is_recurring,
        auto_renew=result.auto_renew,
        raw=result.raw,
    )
